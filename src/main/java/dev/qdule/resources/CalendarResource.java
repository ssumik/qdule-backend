...
public class CalendarResource {
    ...
    @APIResponse(responseCode = "200", description = "Avaliable times to schedule", content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(implementation = AvaliableScheduleResponse.class)))
    public Response availableSchedule(
            @PathParam("year") int year,
            @PathParam("month") int month,
            @QueryParam("treatmentId") Long treatmentId) {
        List<AvaliableScheduleResponse> response = calendarService.availableSchedule(
                treatmentId,
                year,
                month);
    ...
    public Response scheduledTreatment(
            @PathParam("year") int year,
            @PathParam("month") int month) {
        List<AvaliableScheduleResponse> response = calendarService.scheduledTreatment(
                year,
                month);
        return Response.ok().entity(response).build();
    }
